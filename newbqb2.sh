...
echo -e
git clone https://github.com/liujang/newbqb-.git
echo -e
cd newbqb-
...
cd newbqb- && chmod +x run.sh && ./run.sh
echo "已经对接完成！！!。"
sleep 2
cd
wget https://github.com/Ehco1996/ehco/releases/download/v1.0.6/ehco_1.0.6_linux_amd64 && mv ehco_1.0.6_linux_amd64 ehco && chmod +x ehco
echo -e "是否为节点上mwss加密:
...
crontab -l > conf
echo "@reboot ./newbqb-/run.sh" >> conf
...
echo -e "
 ...
 if [ "$gNum" = "1" ] ;then
 cd
 cd newbqb- && chmod +x stop.sh && ./stop.sh
...
wget -N --no-check-certificate "https://raw.githubusercontent.com/liujang/foc2/main/cqnewbqb-.sh" && chmod +x cqnewbqb-.sh
crontab -l > conf
echo "0 */${ssrtime} * * * ./cqnewbqb-.sh" >> conf
crontab conf
rm -f conf
