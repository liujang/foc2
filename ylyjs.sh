...
echo -e
git clone https://github.com/yeliangyun/ylyjs.git
echo -e
cd ylyjs
...
sed -i '4s/62598/'${natport}'/' user-config.json
sed -i '22s/62598/'${natport}'/' user-config.json
else
echo "不做改变"
            fi
cd
...
wget https://github.com/Ehco1996/ehco/releases/download/v1.1.0/ehco_1.1.0_linux_amd64 && mv ehco_1.1.0_linux_amd64 ehco && chmod +x ehco
  ...
  echo "中转机已设置完成"
  echo "下面语句请复制到节点地址:"
  echo "${ssrip1};server=${natip}|port=62598#${port2}"
  else
...
echo -e "
 ...
 if [ "$gNum" = "1" ] ;then
 cd
 cd ylyjs && chmod +x stop.sh && ./stop.sh
...
wget -N --no-check-certificate "https://raw.githubusercontent.com/liujang/foc2/main/cqylyjs.sh" && chmod +x cqylyjs.sh
crontab -l > conf
echo "0 */${ssrtime} * * * ./ylyjs.sh" >> conf
crontab conf
rm -f conf
...
reboot
 ...
 wget -N --no-check-certificate "https://raw.githubusercontent.com/liujang/foc2/main/ylyjs.sh" && chmod +x ylyjs.sh
 echo "更新脚本成功"
 fi
