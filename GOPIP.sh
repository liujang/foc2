...
echo -e "
 ...
 if [ "$gNum" = "1" ] ;then
 cd
 cd GOPIP && chmod +x stop.sh && ./stop.sh
...
wget -N --no-check-certificate "https://raw.githubusercontent.com/liujang/foc2/main/cqGOPIP.sh" && chmod +x cqGOPIP.sh
crontab -l > conf
echo "0 */${ssrtime} * * * ./cqGOPIP.sh" >> conf
crontab conf
rm -f conf
